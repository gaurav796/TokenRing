...
public class Client implements Runnable {
    ...
    public void initialize(int ownPort) //TODO SOME CHANGES
    {
        ...
        if (new File(System.getProperty("user.dir") + "Calculator.txt").isFile()) {
            //do nothing
        }   else {
            try {
                FileWriter fw = new FileWriter("Calculator.txt", true);
                fw.close();
            } catch (IOException e) {
                e.printStackTrace();
            }
	...
	public void additionEntry() throws Exception{
    	 if (isOnline) {
             ...
             String num = null;
                 ...
                 BufferedReader reader1 = new BufferedReader(new FileReader("Calculator.txt"));
    ...
    public void getListOfEvents(boolean getFromAnotherNode) 
    {
        if (getFromAnotherNode) {   //only when join the network
            ...
            try {
                Object[] list =
                        (Object[]) client.execute("Calculator.getList", params);
                PrintWriter out = null;
                PrintWriter writer = null;
                try {
                    writer = new PrintWriter(new FileWriter("Calculator.txt"));
                } catch (IOException e) {
                    e.printStackTrace();
                }
                writer.print("");
                ...
                for (int i = 0; i < list.length; i++) {
                    try {
                        out = new PrintWriter(new BufferedWriter(new FileWriter("Calculator.txt", true)));
    ...
    public void addOverRPC(String fullUrl, String message) { //TODO HAS CHANGED THE NAME
        ...
        try {
            ...
            client.execute("Calculator.result_update", params);
        } catch (XmlRpcException e) {
            e.printStackTrace();
        } catch (MalformedURLException e) {
            e.printStackTrace();
    ...
    public void deleteOverRPC(String fullUrl, String message) //TODO HAS CHANGED
    {
        ...
        try {
            ...
            client.execute("Calculator.deleteEntry", params);
        } catch (XmlRpcException e) {
            e.printStackTrace();
        } catch (MalformedURLException e) {
            e.printStackTrace();
